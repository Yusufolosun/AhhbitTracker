...
export function DemoSandboxBar() {
  ...
  const handleAdvanceBlocks = (blocks: number, label: string) => {
    ...
    try {
      ...
      void refreshBalance();
      showToast(
        `Time advanced by ${label} (${blocks} blocks)! Check your habits to see state changes.`,
        'info',
      );
    } catch {
  ...
  const handleResetDemo = () => {
    if (
      window.confirm(
        'Are you sure you want to reset the demo sandbox? This will delete all your simulated habits.',
      )
    ) {
